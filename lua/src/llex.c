...
#if 1   // HEX_LITERALS_PATCH || UTF8_LITERALS_PATCH
  ...
  while (ls->current != del) {
    switch (ls->current) {
      ...
      case '\\': {
        ...
        #if 1   // HEX_LITERALS_PATCH        //AK 23-May-05
    ...
    */
              if ( c <= 0x007f ) {  // 1-byte
                ...
                save( ls, 0x80 | ((c>>6)&0x3f) );
                save( ls, 0x80 | (c&0x3f) );
            #if 0   //AK 26-May-05: would anyone really need the rest?  (outside of Selene UTF-8 range anyhow)
              } else if ( c <= 0x03ffffff ) {   // 5-byte
                save( ls, 0xf8 | ((c>>24)&0x03) );
                save( ls, 0x80 | ((c>>18)&0x3f) );
                save( ls, 0x80 | ((c>>12)&0x3f) );
                save( ls, 0x80 | ((c>>6)&0x3f) );
                save( ls, 0x80 | (c&0x3f) );
              } else if ( c <= 0x7fffffff ) {   // 6-byte
                save( ls, 0xfc | ((c>>30)&0x01) );
